...
@	beq sdio_fail
@
@	@ EZ5H_SDSendSDIOCommand2 left us r0 and r1 populated with the sdio clock command, save them for later and
@	@ call it once again as we need an extra clock cycle
@	push {r0,r1}
@	bl EZ5H_SendCommand3
@
@	adr r1,read_sector_data
...
@ BEGIN_ASM_FUNC cardExt_RomSendWriteDataogd
        @ ldrb    r3, [r0, #1]
        @ ldrb    r2, [r0]
        @ lsls    r0, r3, #16
        @ lsrs    r3, r3, #4
